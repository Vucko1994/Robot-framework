...
@task
def move_docs(ctx):
    """Move report.html and log.html to Reports"""
    log = Path('./log.html').rename('./Reports/log.html')
    report = Path('./Reports/report.html').rename('./Reports/report.html')
    dest = Path(log) / 'Reports'
    print(log.absolute())
